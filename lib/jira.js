...
(function() {
  ...
  transitionItem = function(issueId) {
    return jiraCli.listTransitions(issueId, function(transitions) {
      ...
      transitions = transitions.transitions; // JIRA returns transitions within an object
  ...
  addItem = function(project) {
    return dutils.ask("Summary", /.+/, function(summary) {
      return dutils.ask("Description", /.+/, function(description) {
        return jiraCli.getFixVersions(project, function(fixVersions){
          var fixVersionCallBack,addIssueCallback,allowedTypes, allowedFixVersions, index, fversion, _i, _j, _len, _ref, _results;
          
          fixVersions.sort(dutils.itemSorter);
          for (index = _i = 0, _len = fixVersions.length; _i < _len; index = ++_i) {
            version = fixVersions[index];
            jiraCli.pp.prettyPrintFixVersions(version, index + 1);
          }

          allowedVersions = (function() {
                _results = [];
                for (var _j = 1, _ref = fixVersions.length; 1 <= _ref ? _j <= _ref : _j >= _ref; 1 <= _ref ? _j++ : _j--){ _results.push(_j); }
                return _results;
          }).apply(this);

          fixVersionCallBack = function(fversion) {

            return jiraCli.getIssueTypes(function(issueTypes) {
              var index, type, _i, _j, _len, _ref, _results;
              ...
              addIssueCallback = function(type) {
                return jiraCli.addIssue(summary, description, fixVersions[fversion -1].id, issueTypes[type - 1].id, project);
              };
              return dutils.ask("Type", allowedTypes, addIssueCallback, allowedTypes);
            });    
          }; 
          return dutils.ask("FixVersion", allowedVersions, fixVersionCallBack, allowedVersions);          
        });
      });
    });
  };
